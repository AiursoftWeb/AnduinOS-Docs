...
* **Disk Snapshots Manager:** Added `anduinos-btrfs-snapshots-manager`, a native graphical product for immutable Btrfs system snapshots and independent Personal Files history. It supports manual and scheduled snapshots, enforced retention policies, protected snapshots, storage visualization, export, rollback reconciliation, cross-disk recovery and discovery, interrupted-operation recovery, and Nautilus File History actions for earlier or deleted files. Its authorization distinguishes personal-file operations from system recovery and its interface reports storage use, cleanup results, pending size, and operation priority.
* **Driver Center:** Added `anduinos-driver-center`, a central graphical product for graphics, audio, printing, Xbox controller, DKMS, and Secure Boot driver health. It can identify incomplete configurations and provide recovery guidance without exposing unrestricted root access to the interface.
* **Secure Boot Toolkit:** Added `anduinos-secureboot-toolkit` for certificate enrollment, DKMS module signing, and Secure Boot recovery workflows used by the installer, OOBE, Driver Center, and optional hardware packages.
* **YubiKey Security Center:** Added `anduinos-yubikey-manager`, a graphical product for YubiKey-backed GDM login, sudo authentication, passwordless-sudo safeguards, resident SSH credentials, persistent SSH configuration, Passkeys, and Git SSH commit signing.
...
* **Software Store Sources:** `anduinos-dconf-defaults` marks Ubuntu and AnduinOS repositories as official GNOME Software sources, while `anduinos-appstore` recommends `flatseal` for graphical Flatpak permission management on Resolute.
...
* **Windows File Associations:** `anduinos-exe-runner` now uses the standard `com.anduinos.ExeRunner.desktop` identifier and claims only Windows executables and MSI installers, avoiding generic native Linux executable types and unintended association takeover.
...
* **Package Suite:** AnduinOS 2 packages now focus on Resolute and remove obsolete Questing-specific branches.
...
* **Signed Development Workflow:** Package and repository publishing workflows use SSH/YubiKey-backed Git signing, with publication preflight and dependency validation.
...
* **Unique Machine Identity:** The Live image does not retain an active SSH listener or reusable host identity. During installation, the installer removes any Live-session SSH host keys, generates new keys owned by the installed machine, validates the resulting `sshd` configuration, and applies the selected initial SSH state.
