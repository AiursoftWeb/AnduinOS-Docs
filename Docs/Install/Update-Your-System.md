...
The default policy is provided by `anduinos-apt-config` or `anduinos-apt-config-dev`:
...
AnduinOS uses **Offline Updates** to apply system updates outside the desktop session.
...
In [Control Panel](../Applications/System/Control-Panel/Control-Panel.md), open **Programs → Software Source**:

1. Select **Check for Updates** and authenticate when requested.
2. Wait while package indexes are refreshed and available package updates are checked.
3. If updates are available, the button becomes **Install Updates**. Select it to start the package upgrade.
4. Keep the window open until the operation finishes. Expand **Terminal Output** to read progress or report an error.

![Control Panel Software Source window with Check for Updates and expandable Terminal Output](../Applications/System/Control-Panel/images/software-source.png)

These are APT system-package operations. Flatpak application updates remain available through App Store. This window also offers **Switch to Fastest Mirror**; changing the mirror refreshes package indexes but does not itself install package upgrades. See [Select best APT source](./Select-Best-Apt-Source.md).

Restart after an update when needed for the kernel or running applications to use the new code.
